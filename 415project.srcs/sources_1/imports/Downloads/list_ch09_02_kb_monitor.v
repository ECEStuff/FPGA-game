...
module kb_monitor
   ...
   //====================================================
   // 50MHz clk gen
   clk_50m_generator(clk, reset_clk, clk_50m);
   ...
   /* instantiate UART
   uart uart_unit
      (.clk(clk_50m), .reset(reset), .rd_uart(1'b0),
   ...
   umbrella_animate game_unit
         (.clk(clk_50m), .reset(reset), .btn(scan_data), .video_on(video_on), .pix_x(pixel_x), .pix_y(pixel_y), .graph_rgb(rgb_next), .audioOut (audioOut));
